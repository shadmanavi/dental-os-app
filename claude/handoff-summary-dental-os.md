...
**Session focus (19 Sep):** KPI dashboard from Maria's workbook; provider names everywhere instead of OpenDental office codes; procedure notes with a dental-assistant picker; OpenDental-username login built, broken by a real bug on first live run, and fixed; a self-service password change and a `/admin/users` screen so Shad runs `sync` himself; the top nav trimmed to Home with everything else moved to (smaller) home-page tiles. **Separately (a second session, same day):** the consent-form signature question was settled and built — see item under "Done" in §3 and the new row in §7. §3 below is the current todo list — the table in §2 below it is 3 weeks stale and kept only for the older history it still holds; §7 has the detailed changelog and `docs/status.md` has the finer-grained session-by-session log.
...
3. **Maria's 3 unscoped tablet asks** — select-all, recording who took the pictures/x-rays, and a prognosis-notes field. None have enough detail yet to build; need a few sentences each from Shad or Maria.
4. **Should `sync` guess a starting role from OpenDental's own user groups** (`userod.UserGroupNum`) instead of defaulting everyone to front_desk? Offered, not yet answered.
5. **Network access control — design agreed 19 Sep, deliberately not built yet.** Shad explicitly asked to hold this until the office is closed, since it touches login and could lock someone out mid-appointment if something goes wrong live. The agreed design:
...
12. **Rename OpenDental's consent-form Descriptions to match the chart's own procedure categories** (Fillings, Crown & Bridge, Endo, ...) — the only step left to make the new consent-form picker's default selection actually fire; nothing else needs to change for it to start working.
...
- ~~Consent-form signature storage matching Topaz~~ (`FROM-CHAT-2026-09-18-B.md`, now deleted — its question is answered) — read OpenDental's own GPL source to confirm the exact signature algorithm rather than relying on docs; confirmed a real Topaz-style signature cannot be written into a SigBox field through the REST API on any OpenDental version, on any account. Shad ruled out a desktop-side agent (the only way that would work) as too much ongoing maintenance, so the built path is a plain PDF with a picture of the signature on it, filed into Imaging — not a native OpenDental Sheet. Built and confirmed working end to end, live, by Shad: a Consent button on the chart's Diags panel, a picker of the office's real consent forms, the form's actual wording pulled live from OpenDental, a signature pad, a filed PDF, and a note line on every procedure the form covers. One step left, not code: renaming OpenDental's consent-form Descriptions so the picker's default selection starts firing (item 12 above).
...
| od-chart v17, od-plan v13, od-production v7, od-hygiene v16, od-kpi v2, chart page v21 | (18 Sep 2026) Providers read "C. Duong DDS" everywhere: first initial, last name, Suffix column when filled in, RDH for a hygienist without one, no credential otherwise (many providers have a blank Suffix — do not invent DDS). Each Diags row has a Notes link: preview from od-plan v13's note_preview, full text via get_note, saved via set_note as a new procnote version (POST /procnotes, the proven path from the old token set_note) | Shad asked for names not office codes. Not yet verified against a live login |
| Dentures investigation (18 Sep 2026) | All 8 denture tiles active in both offices, category active (diagnosed bucket only), page filter correct, od-chart deployed after the tiles were added. Partials need a permanent tooth lit; full dentures need both quadrants of an arch lit; nothing shows under Existing because the Dentures category only exists in diagnosed | **RESOLVED 19 Sep 2026**: Shad confirmed selecting both upper (or both lower) quadrants brings up the tile — working as designed, not a bug. No Existing-bucket category was needed |
| `supabase/functions/od-plan/index.ts` v14, `app/chart/page.tsx` v22 | (19 Sep 2026) New `assistants` action (OpenDental's `employee` table, not `userod` — an assistant needs no OpenDental login). The note editor's textarea grows an Assistant dropdown above it; picking a name writes "Assistant: Name" as the note's own first line, split back out on open (`splitAssistantLine`/`combineAssistantLine`) so the free-text notes feature never has to know about it | procedurelog has no assistant field and the API's "user" is whoever is signed in to OpenDental, never a typed name — confirmed in an earlier session. This was Shad's recommended fallback |
| New `supabase/functions/od-staff-login/index.ts` v1→v2, `supabase/migrations/20260919120000_023_od_staff_logins.sql`, `app/login/page.tsx` v2→v3 | (19 Sep 2026) Staff sign in with their OpenDental username instead of an invented email. `sync` (admin-only, `is_office_admin` gated, one office at a time) reads `userod`, provisions a Supabase Auth account per visible user — synthetic email `<office>.<username>@dental-os.internal`, one-time random temp password, `front_desk` role by default — and deprovisions (bans) anyone OpenDental has since hidden. `reset_password` issues a new temp password for one login. v2 adds `list_usernames`: the login page's Username field is now a dropdown, read live off OpenDental's own roster (not this app's provisioning ledger), the way OpenDental's own login screen shows one — deliberate trade, the roster is visible before sign-in. The login page computes the synthetic email client-side with no lookup. `list_offices` and `list_usernames` need no session (verified live with just the anon key — 37 usernames back for Downey; `sync` correctly refuses an unauthenticated call) | Confirmed via the API docs and this session, not assumed: OpenDental's API has no login-validation endpoint and never returns a password hash, so "the same password" cannot come from OpenDental — only the username is reused, and Supabase owns the password. **Investigated and abandoned**: matching the typed password against `userod`'s own password/salt columns via ShortQuery. The harness's own safety classifier correctly refused the column-guessing probe as credential exploration before any query ran, independent of the fact the probe only ever returned redacted lengths, never a real value. Recommendation stands regardless: even where technically reachable, reimplementing another system's password verification outside it is fragile (silently breaks on any OpenDental hashing change) and duplicates a boundary OpenDental's own API deliberately does not expose — **do not revisit this path**. **`sync` has never been run against a real office** — it creates real accounts with real temp passwords, deliberately left for Shad or a future session to trigger on purpose. No forced-password-change-on-first-login flow yet, and no way to email the temp password to anyone (dental-os.internal isn't deliverable) — both are known gaps, not started |

| `app/login/page.tsx` v4, `app/components/TopNav.tsx` v6, new `app/admin/users/page.tsx` v1 | (19 Sep 2026) Fixed a real regression before it bit anyone: v3's login page had no field left that could carry a raw email, and Shad's own admin account (`shad.manavi@mydentalmasters.com`) predates OpenDental-username login entirely — it would have locked him out. Added a "Sign in with email instead" toggle. TopNav gets a Change Password control (`supabase.auth.updateUser`, needs only the current session) and an Admin nav link. New `/admin/users`: office picker scoped to offices the signed-in user is `owner_admin` at, a roster table with a role dropdown (direct `user_office_roles` write, RLS-enforced), an OpenDental-login column, a Sync button that shows new logins' one-time temp passwords in place, and a per-row Reset password button | This is how `sync` actually gets run from here on — Shad clicks it himself, logged in as himself, not by asking Claude to invoke it directly (which would have needed bypassing the admin check, since Claude has no session). Verified live: dropdowns populate with real Downey names (37), email fallback works, `/admin/users` correctly bounces to `/login` with no session |
| Password-matching investigation (19 Sep 2026) | Shad asked whether the login could validate against OpenDental's own password table. Investigated live: a probe to discover `userod`'s password/salt column names was refused by the harness's own safety classifier as credential exploration, before any query ran — correct to block regardless of the probe's redacted-output design. See [[dental-os-auth-design]] memory | **Closed, do not revisit.** Even where technically reachable, reimplementing OpenDental's own password verification outside their system is fragile (breaks silently on any hashing change) and duplicates a boundary their API deliberately does not expose |
| `supabase/functions/od-staff-login/index.ts` v3, `app/admin/users/page.tsx` | (19 Sep 2026) **Fixed a real bug from the first live `sync` run against Downey.** A trigger (`trg_on_auth_user_created` / `handle_new_auth_user()`) already inserts `public.users` on every new Auth account; `sync`'s own plain `.insert()` collided with it every single time, leaving 37 real Auth accounts with real, unrecoverable temp passwords, no role, no ledger entry. Fixed with `upsert`. New `cleanup_orphans` action removes exactly what the bug left behind (Auth accounts under the office's synthetic domain with no ledger row — cascade-deletes the `users` row too, confirmed via `pg_constraint`), surfaced as a "Clean up broken accounts" button on `/admin/users` when Sync reports the old failure | **The 37 broken Downey accounts are still there, unreachable.** Shad needs to run Clean up broken accounts then Sync again on `/admin/users` before this feature actually works |
| `app/page.tsx` v6, `app/components/TopNav.tsx` v7 | (19 Sep 2026) Nav pared to just Home — `showInTopNav: false` on every other section, "until a menu grouping is decided." Sections still drive "current page" and subnav (Fee Schedules keeps its Upload/Staged uploads tabs), only the bar itself is affected. Home page gains KPI and Admin tiles (7 total), every tile shrunk (tighter padding, smaller type, 3 columns, no "Open" line) | Shad's request, direct UI ask, not a bug fix |
| New `supabase/functions/od-consent/index.ts`, new `lib/consentPdf.ts`, `lib/treatmentPlanPdf.ts` v8, `app/chart/page.tsx` v23, `od-chart` v18 | (19 Sep 2026, separate session) The consent-form feature, built end to end. A Consent button on the chart's Diags panel lists the office's real, live OpenDental consent forms (`list_forms`) and pre-selects one once its OpenDental name is renamed to match the chart's own procedure category (Fillings, Crown & Bridge, Endo, ...) — nothing renamed yet, so nothing defaults today, but the matching itself is confirmed working live. Tooth number (or a notes field) can be entered; the form's real wording is pulled straight from OpenDental (`get_form_text`); a signature is captured; a PDF is built (reusing the treatment-plan PDF's letterhead) and filed under the office's existing "Consent Forms" Imaging category (`upload_signed_form`); every procedure the form covers gets a new "Consent Form Signed (...) — Presented by `<name>`" line on its note, kept separate from the `Assistant:` line since the presenter (whoever hands over the tablet) is not necessarily the assistant. The "Presented by" name reuses the same office-wide presenter picker the treatment-plan flow already has. `od-chart` v18 also returns `patient.Language` so the picker's English/Spanish toggle defaults correctly — checked live first: OpenDental stores a short code (`spa`), not the word "Spanish". | Settles the question three sessions had left open (`FROM-CHAT-2026-09-18-B.md`, now deleted). Read OpenDental's own GPL source directly to confirm no version of the REST API can write a real signature into a SigBox field — this was verified twice independently, not assumed. Shad ruled out the only alternative (a desktop-side agent) as too much ongoing maintenance, so this is a PDF with a signature image, not a native OpenDental Sheet. Confirmed working end to end, live, by Shad. Full detail in `docs/status.md`. |
