...
| New `supabase/functions/od-staff-login/index.ts` v1, `supabase/migrations/20260919120000_023_od_staff_logins.sql`, `app/login/page.tsx` v2 | (19 Sep 2026) Staff sign in with their OpenDental username instead of an invented email. `sync` (admin-only, `is_office_admin` gated, one office at a time) reads `userod`, provisions a Supabase Auth account per visible user — synthetic email `<office>.<username>@dental-os.internal`, one-time random temp password, `front_desk` role by default — and deprovisions (bans) anyone OpenDental has since hidden. `reset_password` issues a new temp password for one login. The login page computes the same synthetic email client-side with no lookup. `list_offices` needs no session (verified live: works with just the anon key; `sync` correctly refuses one) | Confirmed via the API docs and this session, not assumed: OpenDental's API has no login-validation endpoint and never returns a password hash, so "the same password" cannot come from OpenDental — only the username is reused, and Supabase owns the password. **`sync` has never been run against a real office** — it creates real accounts with real temp passwords, deliberately left for Shad or a future session to trigger on purpose. No forced-password-change-on-first-login flow yet, and no way to email the temp password to anyone (dental-os.internal isn't deliverable) — both are known gaps, not started |
