...
//   v13 A quadrant procedure can be written across several quadrants in
//       one call.
//
//       Scaling and root planing is done four quadrants at a time far
//       more often than one, and four calls from the tablet meant four
//       round trips and four chances for the screen and OpenDental to
//       disagree about what happened. commit now takes a regions array
//       and fans the procedure out across every quadrant in it.
//
//       Only quadrant tiles fan out. An arch is one procedure covering
//       an arch, and whole-mouth work has no region at all, so both
//       still write exactly one set of lines.
//
//       Each quadrant is written independently and a refusal does not
//       stop the rest. OpenDental cannot be trusted to delete a row
//       once written, so rolling back is not an option: three quadrants
//       that succeeded stay, and the response names the one that did
//       not. Every line comes back with the quadrant it carries, so the
//       screen can list them separately and undo them separately.
//
//       region is still accepted on its own, so a caller built against
//       v12 keeps working unchanged.
//
...
// A caller may send one region or a list of them. Both arrive here as a
// deduplicated list in OpenDental's own order, so upper right always
// comes before lower left however the buttons were tapped.
function normalizeRegions(list: unknown, single: unknown): string[] {
  const seen = new Set<string>();

  if (Array.isArray(list)) {
    for (const raw of list) {
      const r = normalizeRegion(raw);
      if (r !== "") seen.add(r);
    }
  }

  const one = normalizeRegion(single);
  if (one !== "") seen.add(one);

  return [...QUADRANTS, ...ARCHES].filter((r) => seen.has(r));
}

// "Upper right" from "UR", so a message reads like a sentence.
...
function regionWords(region: string): string {
  return REGION_WORDS[region] ?? region;
}
...
Deno.serve(async (req: Request) => {
  ...
  if (action === "commit") {
    ...
    // empty string means no region field is written at all.
    ...
    if (shape === "tooth") {
      ...
      effectiveRegions = quads;
    } else if (shape === "arch") {
      const arches = regions.filter((r) => ARCHES.includes(r));

      // An arch procedure covers one arch. Two would be two procedures,
      // and nobody has asked for that, so it is refused rather than
      // guessed at.
      if (arches.length !== 1) {
        return json({
          ok: false,
          error: `${tile.label} is charted by arch. Pick either the upper or the lower, not both.`,
        }, 400);
      }
    ...
    // A quadrant tile writes the same lines once per quadrant, so the
    // label says which one. A single-region call reads exactly as it
    // did before.
    const manyRegions = effectiveRegions.length > 1;

    function labelFor(base: string, region: string): string {
      return manyRegions && region !== ""
        ? `${base} — ${regionWords(region)}`
        : base;
    }
    ...
    // One set of lines per region. A tooth or whole-mouth call runs
    // this once with an empty region and produces exactly what v12 did.
    for (const region of effectiveRegions) {
      // Base. Surfaces belong to this line only.
      const base = basePayload(resolved.procCode, region);
      ...
      planned.push({
        role: "base",
        label: labelFor(tile.label, region),
      ...
      if (isPaired) {
        const delivery = basePayload(deliveryCode, region);
        delivery.ProcFee = split === null ? 0 : split.delivery;

        planned.push({
          role: "delivery",
          label: labelFor(`${tile.label} — delivery`, region),
      ...
      for (const addon of chosenAddons) {
        ...
        planned.push({
          role: "addon",
          label: labelFor(String(addon.label ?? code), region),
          proc_code: code,
          addon_id: String(addon.id ?? ""),
          region,
          payload: basePayload(code, region),
        });
      }
    }
    ...
    // Write them.
    //
    // Within one region the lines depend on each other — a crown and its
    // seat — so a refusal abandons the rest of that region. Across
    // regions they do not: the upper left being refused says nothing
    // about the lower right, so the next quadrant is still attempted.
    //
    // Nothing is ever rolled back. OpenDental has been observed refusing
    // to delete rows at some statuses, so an automatic undo cannot be
    // relied on and a half-written commit is reported rather than
    // reversed.
    // -----------------------------------------------------------------
    type WrittenLine = {
      role: string;
    ...
    const failures: { line: PlannedLine; detail: unknown }[] = [];
    const abandoned = new Set<string>();

    for (const line of planned) {
      // An earlier line in this same region was refused, so the rest of
      // that region is not attempted.
      if (abandoned.has(line.region)) continue;

      const created = await odFetch(auth, "POST", "/procedurelogs", line.payload);

      if (created.http_status < 200 || created.http_status >= 300) {
        failures.push({ line, detail: created.body });
        abandoned.add(line.region);
        continue;
      }
    ...
    if (failures.length > 0 && written.length === 0) {
      return json({
        ok: false,
    ...
    // Which quadrants actually landed, for a message the screen can show
    // without working it out itself.
    const regionsWritten = effectiveRegions.filter((r) =>
      r !== "" && written.some((l) => l.region === r)
    );

    const regionsRefused = effectiveRegions.filter((r) =>
      r !== "" && failures.some((f) => f.line.region === r)
    );
    ...
    return json({
      ...
      // The first refusal, flat, so a caller built against v12 keeps
      // reading the shape it expects.
      partial_failure: failures.length === 0 ? null : {
        role: failures[0].line.role,
        label: failures[0].line.label,
        proc_code: failures[0].line.proc_code,
        region: failures[0].line.region,
        detail: failures[0].detail,
      },
      failures: failures.map((f) => ({
        role: f.line.role,
        label: f.line.label,
        proc_code: f.line.proc_code,
        region: f.line.region,
        region_words: regionWords(f.line.region),
        detail: f.detail,
      })),
      regions_written: regionsWritten,
      regions_refused: regionsRefused,
      ...
      surf: baseLine?.surf ?? effectiveRegions[0],
